...
reactable(
  ...
  defaultSortOrder = "desc",
  theme = reactableTheme(
    headerStyle = list(backgroundColor = "#82b1a5", color = "white", fontWeight = "bold")
  )
)
```
...
# 1. Definir a ordem das barras pela Frequência (do maior para o menor)
...
# 2. Calcular o Fator de Transformação para o Eixo Y Secundário
# Isso é necessário para alinhar a linha de taxa (pequenos valores) com a barra de frequência (grandes valores).
...
grafico_severidade <- ggplot(df_plot, aes(x = `Condição Meteorológica`)) +
  ...
  # A. Gráfico de Barras (Eixo Y Esquerdo: Frequência)
  geom_col(aes(y = `Frequência`, fill = `Condição Meteorológica`), 
           show.legend = FALSE, 
           alpha = 0.8) +
  
  # B. Linha de Severidade (Eixo Y Direito: Mortos/Acidente)
  geom_line(aes(y = `Mortos/Acidente` * TAXA_FATOR), # Aplica o Fator
            color = "#DC143C", 
            linewidth = 1.2, 
            group = 1) + 
  ...
  # C. Eixo Y Duplo
  scale_y_continuous(
    # Nome do Eixo Principal (Frequência)
    name = "Frequência de Acidentes",
    # Define o Eixo Secundário
...
reactable(
  ...
  defaultSortOrder = "desc",
  theme = reactableTheme(
    headerStyle = list(backgroundColor = "#82b1a5", fontWeight = "bold")
  )
)
```
...
reactable(
  ...
  bordered = TRUE,
  theme = reactableTheme(
    headerStyle = list(backgroundColor = "#82b1a5", fontWeight = "bold", color = "white")
  )
)
```
...
```{r}
# --- Plot Sugerido 2: Mapa de Calor de Acidentes (Dia da Semana x Fase do Dia) ---

heatmap_data <- Acidentes_2020 |>
  # 1. Garantir a ordem correta dos dias e fases
  mutate(
  ...
  # 2. Contar o número de acidentes para cada combinação de dia e fase
  count(dia_semana_fator, fase_dia_fator, name = "total_acidentes") |>
  
  # 3. Agrupar os dados por dia da semana para calcular a proporção diária
  group_by(fase_dia_fator) |>
  
  # 4. Calcular a proporção de acidentes para cada fase DENTRO de cada dia
  # A proporção é o total da fase / soma de todas as fases daquele dia
  mutate(proporcao = total_acidentes / sum(total_acidentes)) |>
  
  # 5. (Opcional, mas recomendado) Desagrupar para evitar que afete futuras manipulações
  ungroup()
...
reactable(
  ...
  columns = list(
    ...
    total_acidentes = colDef(
      ...
      align = "left",
      minWidth = 200,
      # Função 'cell' para criar a barra de frequência
      cell = function(value) {
        # Calcula a largura da barra em relação ao valor máximo
        width <- paste0(value / max(acidentes_por_hora$total_acidentes) * 100, "%")
        bar_color <- "#82b1a5" # Um azul vibrante
        
        # HTML para desenhar o número e a barra
        div(
          # Define a barra como um gradiente de cor
          style = list(background = paste0("linear-gradient(90deg, ", bar_color, " ", width, ", transparent ", width, ")"),
                       padding = "4px 8px", 
                       margin = "-4px 0"), 
          value
...
ggplot(acidentes_por_hora, aes(x = hora_dia, y = total_acidentes)) +
  geom_col(fill = "#82b1a5", alpha = 0.8) +
...
print(plot_bar_mortos)
ggsave("plot_bar_mortos_por_acidente.pdf", plot = plot_bar_mortos, width = 12, height = 8)
```
